        ...
        <div class="row">
            <div class="col-md-6 col-sm-12 px-5">
                ...
                <div class="text-lg-right text-center  mx-5 my-1">
                    ...
                    <a class="pl-lg-5 mx-5 MontserratArabic-logo text-warning login-logo text-decoration-none" href="{{route('index')}}">ترزي</a>
                    <p class="pr-lg-5  pt-3 MontserratArabicLight font30">
                        تعيين كلمة مرور جديدة
                    </p>
                </div>

                <form method="POST" action="{{ route('vendor.password.update') }}" class="form justify-content-center px-lg-5 px-sm-2 mx-lg-5  ml-lg-5 reg-form-custom" _lpchecked="1">
                    @csrf
                    <input type="hidden" name="token" value="{{ $token }}">

                    <div class="row" style="height: 70px">
                        <div role="group" class="col-md-12 col-sm-12">
                            <input name="email" id="email"  type="email" class="form-control" value="{{ $email or old('email') }}" placeholder=" ">
                            <label for="email" class="MontserratArabicLight label">البريد الالكتروني</label>
                            @error('email')
                            <span class="text-danger MontserratArabicLightPure float-right position-absolute span" role="alert">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div class="row mt-1" style="height: 70px">
                        <div role="group" class="col-md-12 col-sm-12">
                            ...
                            <label for="password" class="MontserratArabicLight label">كلمة السر</label>
                            ...
                            <div class="row position-absolute">
                                ...
                                @else
                                    <span><small class="float-right MontserratArabicLightPure mr-3">كلمة السر يجب ان تكون ما بين 4 الى 6 احرف</small></span>
                    ...
                    <div class="row mt-1" style="height: 70px">
                        <div role="group" class="col-md-12  col-sm-12">
                            ...
                            @error('password')
                            <span class="text-danger MontserratArabicLightPure float-right span">{{$message}}</span>
                            @enderror
                        </div>
                    </div>
                    <div class="row">
                        <button type="submit" class="btn btn-yellow2 w-100 m-3 MontserratArabicLight">تعيين كلمة المرور الجديدة </button>
                    </div>
                </form>
